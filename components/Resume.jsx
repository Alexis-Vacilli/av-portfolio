...
const training = [
  {
    ...
    date: "2020 - Present",
    description:
      "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Fuga quibusdam quas id cumque voluptate iusto vitae officia quam inventore reprehenderit.",
  },
  {
    title: "Mobile App Developer",
    company: "Andela",
              ...
              {training.map((item, index) => {
                return (
                  <AccordionItem
                    key={index}
                    value={`item-${index}`}
                    className="centered text-xl font-semibold border-b-2 border-foreground dark:border-border"
                  >
                    <AccordionTrigger className="px-6 py-8">
                      {item.title}
                    </AccordionTrigger>
                    <AccordionContent>
                      <div className="centered flex justify-between items-center ">
                        <span className="text-3xl secondary-font">
                          {item.company}
                        </span>
                        <span className="bg-primary">{item.date}</span>
                      </div>
                      <div className="centered text-start py-4 font-normal">
                        {item.description}
                      </div>
                    </AccordionContent>
                  </AccordionItem>
